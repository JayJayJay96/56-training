...
1. 已經大概可以分到 Web、Core、Infrastructure 各自做什么。
2. 也有找到一个 Gold discount 被算两次的问题, agent 也有給 summary 但是不是很清楚。
3. 之后加功能会先跟著现有 pattern，不会直接在 Controller 里面塞 business logic。
